...
pub async fn seed_movimentacao_robusta() {
    ...
    for dias_atras in 0..dias_loop {
        ...
        let claims = crate::services::transacao::Claims {
            sub: id_user.clone(),
            email: "seed_user@teste.com".to_string(),
            exp: 2000000000,
        };
        ...
        for i in 0..entradas {
            ...
            let categoria_id = if i % 2 == 0 {
                id_categoria_uber.clone()
            } else {
                id_categoria_99.clone()
            };
        ...
        for i in 0..saidas {
            ...
            let categoria_id = if i % 2 == 0 {
                id_categoria_abastecimento.clone()
            } else {
                id_categoria_alimentacao.clone()
            };
            let transacao_payload = CreateTransacaoPayload {
                id_categoria: categoria_id,
                valor: (valor_val * 100.0).round() as i32,
        ...
        diesel
            ::insert_into(dsl::sessoes_trabalho)
            .values(&nova_sessao)
            .execute(conn)
