        ...
        <Card sx={{ maxWidth: 400, width: '100%', backgroundColor: 'background.paper', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
          <CardContent sx={{ p: 4 }}>
            ...
            <Box component="form" onSubmit={handleSubmit}>
              {activeStep === 0 && (
                <>
                  <TextField id="nome_usuario" fullWidth label="Nome de usuário" type="text" value={state.nome_usuario} onChange={e => setField('nome_usuario', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
                  <TextField id="email" fullWidth label="E-mail" type="email" value={state.email} onChange={e => setField('email', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
                  <TextField id="nome_completo" fullWidth label="Nome completo" type="text" value={state.nome_completo} onChange={e => setField('nome_completo', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
                  <TextField id="telefone" fullWidth label="Telefone" type="text" value={state.telefone} onChange={e => setField('telefone', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
                  <TextField id="cpfcnpj" fullWidth label="CPF ou CNPJ" type="text" value={state.cpfcnpj} onChange={e => setField('cpfcnpj', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
              ...
              {activeStep === 1 && (
                <>
                  ...
                  <TextField id="address" fullWidth label="Endereço" type="text" value={state.address} onChange={e => setField('address', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
                  <TextField id="address_number" fullWidth label="Número" type="text" value={state.address_number} onChange={e => setField('address_number', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
                  <TextField id="complement" fullWidth label="Complemento" type="text" value={state.complement} onChange={e => setField('complement', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
                  <TextField id="province" fullWidth label="Bairro" type="text" value={state.province} onChange={e => setField('province', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
                  <TextField id="city" fullWidth label="Cidade" type="text" value={state.city} onChange={e => setField('city', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} />
              ...
              {activeStep === 2 && (
                <>
                  <TextField id="senha" fullWidth label="Senha" type={state.showPassword ? 'text' : 'password'} value={state.senha} onChange={e => setField('senha', e.target.value)} sx={{ mb: 2 }} disabled={state.loading} InputProps={{ endAdornment: (<InputAdornment position="end"><IconButton aria-label={state.showPassword ? 'Ocultar senha' : 'Mostrar senha'} onClick={() => setField('showPassword', !state.showPassword)} edge="end" tabIndex={-1}>{state.showPassword ? <VisibilityOff /> : <Visibility />}</IconButton></InputAdornment>) }} />
                  <TextField id="confirmSenha" fullWidth label="Confirme a senha" type={state.showPassword ? 'text' : 'password'} value={state.confirmSenha} onChange={e => setField('confirmSenha', e.target.value)} sx={{ mb: 3 }} disabled={state.loading} />
                  {/* Captcha */}
                  <Box sx={{ mb: 2, textAlign: 'center' }}>
                    ...
                    <TextField id="captcha" fullWidth label="Digite o texto da imagem" type="text" value={state.captchaAnswer} onChange={e => setField('captchaAnswer', e.target.value)} sx={{ mt: 1 }} disabled={state.loading || state.captchaLoading} />
                  </Box>
