...
export const GoalProgress: React.FC<GoalProgressProps> = ({ meta, isActive }) => {
  ...
  const now = toBackendLocalString(getCurrentDateTime(), userTimezone);
  ...
  const payload = { eh_concluida: false, concluida_com: totalAtingido, atualizado_em: toBackendLocalString(getCurrentDateTime(), userTimezone) };
      void persistUpdate(payload);
    }
  ...
  const nowIso = toBackendLocalString(getCurrentDateTime(), userTimezone);
  ...
  }, [isConcluida, isExpirada, totalAtingido, dispatchMetas, meta, isMetaEconomia, userTimezone]);
