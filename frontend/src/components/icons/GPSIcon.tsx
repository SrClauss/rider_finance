...
type GpsIconProps = SVGProps<SVGSVGElement> & {
  fill?: string;
  scale?: number; // multiplicador de escala, padrão 1
  crop?: boolean; // se true, usa 'slice' para eliminar espaços vazios no viewBox
  preserve?: string; // opcional: valor direto para preserveAspectRatio
  gradient?: string[]; // array de cores para criar um gradiente
  gradientId?: string; // id opcional para o gradiente (se não informado, é gerado)
  gradientType?: 'linear' | 'radial';
  gradientRotation?: number; // degrees, apenas para linear
};
const GpsIcon = ({ fill = '#000', scale = 1, width, height, crop = true, preserve, gradient, gradientId, gradientType = 'linear', gradientRotation = 90, style, ...props }: GpsIconProps) => {
  // tamanho padrão base (px) — ajustável por `scale` ou por `width`/`height` passados
  // aumentamos o base para 48 para evitar corte em containers pequenos
  const base = 12;
  const w = width ?? Math.round(base * scale);
  const h = height ?? Math.round(base * scale);
  // mescla estilo passado via props com display:block para remover gap de baseline
  // adicionamos overflow: hidden para evitar que áreas vazias sejam renderizadas fora do box
  const mergedStyle = { ...(style as React.CSSProperties || {}), display: 'block', overflow: 'hidden' } as React.CSSProperties;
  // quando `crop` estiver true usamos `slice` para preencher e recortar o viewBox, eliminando padding interno
  // `preserve` (se fornecido) tem preferência e permite testes rápidos: 'none' | 'xMidYMid slice' | etc.
  const preserveVal = (preserve as string) ?? (crop ? 'xMidYMid slice' : 'xMidYMid meet');
  ...
  if (!idRef.current) {
    ...
    const seed = JSON.stringify({ gradient, gradientType, gradientRotation, width: w, height: h });
    idRef.current = gradientId ?? `gps-grad-${simpleHash(seed)}`;
  }
  const generatedId = idRef.current!;

  // se houver um gradiente, montamos o defs e definimos o fill como url(#id)
  const hasGradient = Array.isArray(gradient) && gradient.length > 0;
  const fillValue = hasGradient ? `url(#${generatedId})` : fill;

  return (
    <svg width={w} height={h} viewBox="0 0 310 310" preserveAspectRatio={preserveVal} xmlns="http://www.w3.org/2000/svg" {...props} style={mergedStyle}>
      {hasGradient && (
        <defs>
          {gradientType === 'linear' ? (
            <linearGradient id={generatedId} gradientUnits="objectBoundingBox" gradientTransform={`rotate(${gradientRotation})`}>
              {gradient!.map((c, i) => (
                <stop key={i} offset={`${(i / (gradient!.length - 1)) * 100}%`} stopColor={c} />
              ))}
            </linearGradient>
          ) : (
            <radialGradient id={generatedId} gradientUnits="objectBoundingBox">
              {gradient!.map((c, i) => (
                <stop key={i} offset={`${(i / (gradient!.length - 1)) * 100}%`} stopColor={c} />
              ))}
            </radialGradient>
